...
import { scheduleJobReminders, cancelJobReminders } from '../utils/notifications';
...
const EFFORT_OPTIONS: { value: ApplicationEffort; label: string; desc: string }[] = [
  { value: 'bespoke', label: '🎯 Bespoke', desc: 'Fully tailored' },
  { value: 'template', label: '📋 Template', desc: 'Adapted from base' },
...
export default function JobFormModal({ job, onClose, onSave }: Props) {
  ...
  const extractFieldsFromOCR = (rawText: string) => {
    ...
    if (lines[0]) setForm((p) => ({ ...p, jobTitle: p.jobTitle || lines[0].slice(0, 80) }));
  ...
  const addCheckItem = () => {
    ...
    if (t) {
      set('checklist', [...form.checklist, { id: uuidv4(), text: t, done: false } as ChecklistItem]);
      setNewCheckItem('');
    }
  };
  ...
  const addContact = () => {
    if (!newContact.name.trim()) { toast.error('Contact name is required'); return; }
    set('contacts', [...contacts, { id: uuidv4(), ...newContact } as Contact]);
  ...
  const addReminder = (timing: Reminder['timing']) => {
    set('reminders', [...form.reminders, { id: uuidv4(), timing, triggered: false } as Reminder]);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEdit && job?.id) {
        ...
        await cancelJobReminders({ ...form, id: job.id });
        await scheduleJobReminders({ ...form, id: job.id });
        ...
        await scheduleJobReminders({ ...form, id: newId as number });
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
            ...
            <>
              <div>
                ...
                <div
                  ...
                  className={`border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-all ${
                    isDragActive ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-950'
                      : 'border-slate-200 dark:border-gray-700 hover:border-indigo-400'
                  }`}
                >
                  ...
                  {ocring ? (
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-amber-600 dark:text-amber-400">
                        ⚠️ Best results: direct screenshot of advert text, not a photo of a screen
                      </p>
                      <button
              ...
              <div className="space-y-3 pt-2 border-t border-slate-100 dark:border-gray-800">
                ...
                <div>
                  ...
                  <textarea className={textAreaCls} rows={6} placeholder="Main duties and role overview..."
          ...
          {activeTab === 'tracking' && (
            <>
              ...
              <div className="bg-slate-50 dark:bg-gray-800 rounded-xl p-4 space-y-3">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {EFFORT_OPTIONS.map((opt) => (
                      <button key={opt.value} onClick={() => set('applicationEffort', opt.value as ApplicationEffort)}
                ...
                <div>
                  <label className={labelCls}><Clock className="w-3 h-3 inline mr-1" />Hours Spent</label>
                  <div className="flex items-center gap-3">
                    <input type="number" min={0} step={0.5} className={inputCls + ' w-28'} placeholder="0"
                  ...
                  {form.checklist.map((item) => (
                    <div key={item.id} className="flex items-center gap-2 p-2 bg-slate-50 dark:bg-gray-800 rounded-lg">
                      <button onClick={() => toggleCheck(item.id)}>
                        {item.done ? <CheckSquare className="w-4 h-4 text-indigo-600" /> : <Square className="w-4 h-4 text-gray-400" />}
                      </button>
            ...
            <div className="space-y-4">
              <div className="bg-amber-50 dark:bg-amber-950/50 rounded-xl p-3 border border-amber-100 dark:border-amber-900">
                <p className="text-xs text-amber-700 dark:text-amber-300 font-medium">
                  🔍 Research the successful candidate via LinkedIn or public sources
                </p>
              </div>
              {[
                { key: 'whoGotHired', label: 'Who Eventually Got Hired', placeholder: 'Name or description...' },
                ...
                { key: 'skillsTheyHad', label: 'Skills They Had That I Lacked', placeholder: 'Technical or soft skills...' },
                { key: 'experienceLevelFit', label: 'Experience Level / Industry Fit', placeholder: 'How their experience aligned...' },
          ...
          {activeTab === 'contacts' && (
            <div className="space-y-4">
              <div className="bg-indigo-50 dark:bg-indigo-950/50 rounded-xl p-3 border border-indigo-100 dark:border-indigo-900">
                <p className="text-xs text-indigo-700 dark:text-indigo-300 font-medium">
                  🤝 Track people behind this opportunity — hiring managers, referrals, internal contacts
                </p>
              </div>
                ...
                <div className="bg-slate-50 dark:bg-gray-800 rounded-xl border border-indigo-200 dark:border-indigo-800 p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="col-span-2">
                      ...
                      <input className={inputCls} placeholder="Full name" value={newContact.name}
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => { setShowContactForm(false); setNewContact(EMPTY_CONTACT); }}
                      className="px-4 py-2 bg-slate-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-sm rounded-xl">
                      Cancel
                    </button>
                  </div>
                </div>
            ...
            <div className="space-y-4">
              <div className="bg-blue-50 dark:bg-blue-950/50 rounded-xl p-3 border border-blue-100 dark:border-blue-900">
                <p className="text-xs text-blue-700 dark:text-blue-300 font-medium">
                  🔔 Notifications fire immediately when you save — no need to reopen the app. Also check: Settings → Apps → JobTrackr → Battery → set to <strong>Unrestricted</strong>
                </p>
              </div>
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    <button key={timing} onClick={() => addReminder(timing)}
              ...
              {form.status === 'wishlist' && (
                <div className="bg-blue-50 dark:bg-blue-950/50 rounded-xl p-3 border border-blue-100 dark:border-blue-900">
                  <p className="text-xs text-blue-700 dark:text-blue-300">
                    💡 Wishlist job — use <strong>Remind Me to Apply</strong> to set a date to take action
                  </p>
                </div>
              )}
